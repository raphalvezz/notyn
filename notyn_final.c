...
void menu()
{
	...
	printf("[5] para cadastrar uma nova pessoa\n");
	printf("[6] para cadastrar uma pessoa em um Evento\n");
	printf("[0] para sair\n");
}
...
struct PESSOAS{
	...
	int idade_usuario,i,j,opcao,horario_cadastrado,dia_cadastrado,mes_cadastrado,dia_atual,mes_atual,cont = 0, total_eventos = 0, contador = 0, total_pessoas = 0, tamanho_eventos, tamanho_pessoas, tamanho_ep=0;
...
int main(){
	...
	struct PESSOAS p[MAX_QUANTIDADE] = {{"ana",19,'f','e',"balada,rodeio,bar"}, 
			...
			case 6:
				...
				do{
						do{
							...
							scanf("%d",&pe[i].codigo);
						}while(pe[i].codigo<1 || tamanho_eventos>pe[i].codigo);
